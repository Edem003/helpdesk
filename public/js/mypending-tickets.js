        ...
        success: function(response)
        {
            for(let i = 0; i<response.data.length; i++)
            {
                ...
                $.ajax({
                    ...
                    {
                        var csrf = data;
                        document.getElementById("csrf_token_mypending_a${response.data[i].id}").value = csrf;
                        document.getElementById("csrf_token_mypending_b${response.data[i].id}").value = csrf;
                        document.getElementById("csrf_token_mypending_c${response.data[i].id}").value = csrf;
                    }
                })

                $(document).ready(function() {
  
                    $('#mypending_ticket').on('click','.details_${response.data[i].id}',function(){
                      ...
                      $.ajax({
                        ...
                        success: function(results)
                        {
                          ...
                          document.getElementById('note_id_c${response.data[i].id}').innerHTML = html;
                        }
                      });
                    });
                  })
                ...
                <div class="text-center">
                    <a href="#" class="btn btn-outline-dark btn-xs details_${response.data[i].id}" type="button" data-bs-toggle="modal" data-bs-target="#pendingmodal${response.data[i].id}">
                        <i class="fas fa-info fa-sm" data-container="body" data-bs-toggle="tooltip" data-bs-placement="left" title="View Details"></i>
                    </a>
                </div>
                <div class="modal fade" id="pendingmodal${response.data[i].id}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-xl">
                        <div class="modal-content">
                        <div class="modal-header">
                            <div>
                            <ul class="nav main-menu" role="tablist">
                                <li class="me-2"><a class="btn btn-primary-light btn-sm" id="pills-info-tab" data-bs-toggle="pill" href="#pills-info-pending${response.data[i].id}" role="tab" aria-controls="pills-info" aria-selected="true"><i class="fas fa-info-circle fa-fw"></i> Details</a></li>
                                <li class="me-2"><a class="btn btn-primary-light btn-sm show" id="pills-edit-tab" data-bs-toggle="pill" href="#pills-edit-pending${response.data[i].id}" role="tab" aria-controls="pills-edit" aria-selected="false"><i class="fas fa-edit fa-fw"></i> Edit</a></li>
                                <li class="me-2"><a class="btn btn-primary-light btn-sm show" id="pills-note-tab" data-bs-toggle="pill" href="#pills-note-pending${response.data[i].id}" role="tab" aria-controls="pills-note" aria-selected="false"><i class="fas fa-sticky-note fa-fw"></i>  Add Note</a></li>
                                <li class="me-2"><a class="btn btn-primary-light btn-sm show" id="pills-edit-tab" data-bs-toggle="pill" href="#pills-form-pending${response.data[i].id}" role="tab" aria-controls="pills-edit" aria-selected="false"><i class="fas fa-file-alt fa-fw"></i> Complainant Form</a></li>
                            </ul>
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="tab-content">
                                <div class="tab-pane fade active show" id="pills-info-pending${response.data[i].id}" role="tabpanel" aria-labelledby="pills-info-tab">  
                                <div class="card" style="border-radius: 5px;">
                                    <div class="text-start" style="margin: 10px;">
                                ...
                                <div class="card" style="border-radius: 5px;">
                                    ...
                                    <ul>
                                        <li><p id="note_id_c${response.data[i].id}"></p></li>
                                    </ul>
                                    </div>
                                </div>
                                </div>
                                <div class="fade tab-pane" id="pills-edit-pending${response.data[i].id}" role="tabpanel" aria-labelledby="pills-edit-tab">                                    
                                <div class="card" style="border-radius: 5px;">
                                    <div class="text-start" style="margin: 10px;">
                                    ...
                                    <form class="form-bookmark needs-validation mt-3" method="POST" action="update_handle_ticket" id="bookmark-form" novalidate="">
                                        <div class="mb-3">
                                        <input type="text" id="csrf_token_mypending_a${response.data[i].id}" name="_token" hidden>
                            ...
                            <div class="fade tab-pane" id="pills-note-pending${response.data[i].id}" role="tabpanel" aria-labelledby="pills-note-tab">                                    
                                <div class="card" style="border-radius: 5px;">
                                <div class="text-start" style="margin: 10px;">
                                    <h6 class="text-warning small">NOTE</h6>
                                    ...
                                    <div class="row">
                                    <div class="mb-3">
                                    <input type="text" id="csrf_token_mypending_b${response.data[i].id}" name="_token" hidden>
                                        <label class="col-form-label pt-0" for="exampleInputEmail1">Ticket ID<span class="text-danger">*</span></label>
                                        <input class="form-control" id="exampleInputEmail1" type="text" name="code" value="${response.data[i].code}" readonly required>
                                    </div>
                                    <div class="mb-0 mt-3">
                            ...
                            <div class="fade tab-pane" id="pills-form-pending${response.data[i].id}" role="tabpanel" aria-labelledby="pills-edit-tab">                                    
                                <div class="card" style="border-radius: 5px;">
                                    <div class="text-start" style="margin: 10px;">
                                    <h6 class="text-warning small">FORM DETAILS</h6>
                                    <form class="form-bookmark needs-validation mt-3" method="POST" action="generate_print_form" id="bookmark-form" novalidate="">
                                        <div class="mb-2">
                                        <input type="text" id="csrf_token_mypending_c${response.data[i].id}" name="_token" hidden>
                                            <label class="col-form-label pt-0" for="exampleInputEmail1">Ticket ID:</label>
                                            <input class="form-control" name="code" value="${response.data[i].code}" required readonly>
                                        </div>
                                        <div class="mb-2">
                                            <label class="col-form-label pt-0" for="exampleInputEmail1">Description:</label>
                                            <input class="form-control" name="subject" value="${response.data[i].subject}" required readonly>
                                        </div>
                                        <div class="mb-2">
                                            <label class="col-form-label pt-0" for="exampleInputEmail1">Complainant:</label>
                                            <input class="form-control" name="complainant_name" value="${response.data[i].complainant_name}" required readonly>
                                        </div>
                                        <div class="mb-2">
                                            <label class="col-form-label pt-0" for="exampleInputEmail1">Division/Unit:</label>
                                            <input class="form-control" name="division" value="${response.data[i].division} - ${response.data[i].complainant_office}" required readonly>
                                        </div>
                                        <div class="mb-2">
                                            <label class="col-form-label pt-0" for="exampleInputEmail1">Serial Number:</label>
                                            <input class="form-control" name="serial_no" required>
                                        </div>
                                        <div class="mb-2">
                                            <label class="col-form-label pt-0" for="exampleInputEmail1">IT Officer:</label>
                                            <input class="form-control" name="assigned_user" value="${response.data[i].first_name} ${response.data[i].surname}" required readonly>
                                        </div>
                                        <div class="mb-2">
                                        ...
                                        <div>
                                            <button class="btn btn-primary print" type="submit"><i class="fas fa-arrow-circle-right small"></i> Generate Form</button>
                                        </div>
                                    </div>
                                    </form>
                                </div>
